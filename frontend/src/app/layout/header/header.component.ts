              ...
              @for (result of searchResults; track result.id) {
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    @if (result.reference) {
                      <p class="text-xs text-gray-400">Ref: {{ result.reference }}</p>
                    }
                  </div>
...
export class HeaderComponent {
    ...
    this.searchSubject
      .pipe(
        ...
        switchMap((q) => {
          ...
          return this.productService.quickSearch(q, 8);
        }),
      )
      .subscribe({
        next: (res: any) => {
          ...
          if (res && res.data) {
            this.searchResults = res.data.map((p: any) => ({
              ...
              coverImage: this.getCoverImage(p),
            }));
          }
        },
        error: () => {
  ...
  private getProductName(product: any): string {
    ...
    const trans = translations[0] as any;
    return trans?.name || '';
  }

  private getCoverImage(product: any): string | null {
    const images = Array.isArray(product.images) ? product.images : [];
    const cover = images.find((img: any) => img.cover) || images[0];
    if (!cover?.path) return null;
    if (cover.path.startsWith('http')) return cover.path;
    return environment.apiUrl.replace('/api/v1', '') + '/' + cover.path.replace(/^\//, '');
  }
}
